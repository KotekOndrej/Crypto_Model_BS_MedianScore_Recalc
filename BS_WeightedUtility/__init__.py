...
def exp_decay_weights(n_days: int, half_life: float):
    import numpy as np
    # age 0 = nejnovější den (poslední v historii)
    ages = np.arange(n_days-1, -1, -1)  # starší -> větší age
...
def price_only_features(opens, highs, lows, closes):
    # stejné jako v původním, rozšíření doplníme následně v daily_features()
...
def logistic_regression_irls(X, y, sample_weight=None, max_iter=100, tol=1e-6, reg_lambda=1e-4):
    ...
    for _ in range(max_iter):
        ...
        for a in range(d+1):
            H[a][a] += reg_lambda
            g[a] += reg_lambda * w[a]
        # Solve H * step = g  (simple Gauss-Jordan)
...
def find_local_peaks(y, k_peaks=60, min_separation=2):
    # jednoduchý peak picker (stejně jako dříve)
    peaks = []
...
def daily_stats_1m(df_day):
    """Vrátí dictionary denních statistik z 1m svíček."""
    import numpy as np
    if df_day.empty:
        return None
    ...
    # ATR proxy z minut (True range na min a průměr přes den)
    prev_c = np.concatenate([[c[0]], c[:-1]])
    tr = np.maximum.reduce([h - l, np.abs(h - prev_c), np.abs(prev_c - l)])
    atr_d = float(np.mean(tr))

    # realized volatility (root sum of squares returns)
    ...
    # order-flow imbalance (taker buy vs all)
    imb = 0.0
...
def classify_regime(win_stats):
    """Jednoduchá klasifikace: range / trend / vol podle poměru směru a volatility."""
    import numpy as np
    if len(win_stats) < 5:
        return "range"
    ...
    # trend proxy: EMA(imbalance) a změna VWAP
...
def build_profile_weighted(df_day, bins):
    """Time*volume přiblížení: váha ~ sqrt(quoteVolume) * sqrt(numTrades)."""
    import numpy as np
...
def weighted_percentile(values, weights, p):
    """Přibližný vážený percentil (0-100)."""
    import numpy as np
...
def compute_bs_for_csv_bytes(csv_bytes: bytes, pair_name: str):
    ...
    # rozšíření featur o likviditu a tok (z denních stats)
    feats_basic = price_only_features(list(opens), list(highs), list(lows), list(closes))
    # sjednotíme délky (feats_basic má len = len(history))
    ...
    close_last = closes[-1]
    # base gap z ATR + fallback GAP_MIN_BPS
    gap_min_abs = max(stats_last["atr_d"] * 0.5, close_last * (GAP_MIN_BPS / 10000.0))
    # režimové multiplikátory
    if regime == "trend":
        gap_min_abs *= 1.2
    elif regime == "vol":
        gap_min_abs *= 1.5
    # efektivní náklad: penalizuj nízké numTrades (větší frikce)
    nt_last = max(stats_last["avg_nt"], 1.0)
    costs_pct_eff = BASE_COSTS_PCT * ( (stats_last["avg_nt"] / nt_last) ** 0.5 )  # v rozumných mezích ~1x
    # bez velkých výkyvů – omezíme na [0.5x, 2x]
    ...
    def eval_pair(B, S):
        ...
        for (L, H, _, _, _), wd in zip(history, w_days):
            ...
            pnls.append(p * wd)   # vážíme per den (lineární aproximace na PnL)
            cycles.append(c * wd)
        pnls_raw = [p for p in pnls]  # už vážené
        # robustní metriky
        med = float(np.median(pnls_raw))
        iqr = float(np.percentile(pnls_raw, 75) - np.percentile(pnls_raw, 25))
        maxdd = max_drawdown_path(pnls_raw)
        utility = med - LAMBDA_IQR * iqr - MU_MAXDD * maxdd
        total_cycles = float(np.sum(cycles))
        avg_pnl_pct = float((np.mean(pnls_raw) / base) * 100.0)
        median_pnl_pct = float((med / base) * 100.0)
        gap_pct = (S - B) / max(B, 1e-12)
        dist_rel = abs(((B + S) / 2.0) - current_price) / max(current_price, 1e-12)
        return {
            "utility": utility, "total_cycles": total_cycles, "gap_pct": gap_pct,
            "dist": dist_rel, "avg_pnl_pct": avg_pnl_pct, "median_pnl_pct": median_pnl_pct
        }
    ...
    for (B, S) in pairs:
        met = eval_pair(B, S)
        eval_cache[(B, S)] = met
        # Primárně: vyšší utility; pak více cyklů; menší gap; blíž k ceně
    ...
    # Diagnostika za celé okno (nevážené i vážené hit-rate)
    ...
    return {
        ...
        "model": MODEL_NAME,
        "regime": regime,
        "total_cycles": float(met["total_cycles"]),
        "utility": float(met["utility"]),
        "score": float(met["utility"]),  # pro kompatibilitu ponecháme i 'score'
...
def main(myTimer):
    ...
    for blob in blobs:
        ...
        try:
            ...
            new_rows.append({
                ...
                "model": res["model"],
                "is_active": True,
                "total_cycles": float(res.get("total_cycles", 0.0)),
                # Ponecháváme 'score' pro kompatibilitu, ale ukládáme zároveň 'utility'
            ...
            logging.info(
                ...
                f"cycles={float(res.get('total_cycles',0.0)):.2f}, utility={float(res.get('utility',0.0)):.6f}, "
                f"side={res.get('side')} (p_up={float(res.get('p_up',0.5)):.3f}), regime={res.get('regime')}"
            )
        except Exception:
    ...
    cols = ["pair","B","S","gap_pct","date","model","is_active",
            "total_cycles","score","utility","avg_pnl_pct","median_pnl_pct","load_time_utc",
    ...
    # 4) Master CSV – deaktivace pouze pro shodné (pair, model), ostatní modely ponechat
    master_cols = cols
    try:
